...
dataframe = dataframe[-r(2,7,11,12,14,17,24,25,30), ] # doesn't work

#median of optimized parameters
hr_embed = median(dataframe$HeartEmbed)
hr_delay = median(dataframe$HeartDelay)
hr_radius = median(dataframe$HeartRadius)

resp_embed = median(dataframe$RespEmbed)
resp_delay = median(dataframe$RespDelay)
resp_radius = median(dataframe$RespRadius)
...
ans_hr2 = optimizeParam(x, x, par, min.rec= 3.5, max.rec= 4.5)

Results=crqa (x, x, delay=hr_delay, embed=hr_embed, radius=hr_radius,normalize=0,rescale=0,mindiagline = 2,minvertline = 2) 

#crqa respiration

ans_resp2 = optimizeParam(x, x, par, min.rec= 3.5, max.rec= 4.5)

Results=crqa (x, x, delay=resp_delay, embed=resp_embed, radius=resp_radius,normalize=0,rescale=0,mindiagline = 2,minvertline = 2) 
```
