...
Output: group, condition, trial, hr_rr, hr_det, hr_l, resp_rr, resp_det, resp_l
...
hr_radius=round(median(dataframe$HeartRadius, na.rm=T),2)
...
#function for heart rate
fitCrqa1 <- function(ts1,ts2){
  ...
  if (length(Pars)>1){
    Results=crqa(ts1, ts2, delay=0, embed=16.5, radius=0.38, normalize=0, rescale=0, mindiagline = 2, minvertline = 2)
  } 
  # else {
  #   Results=NULL
  #   Pars=NULL
  ...
  df=data.frame(RR=Results$RR,DET=Results$DET,LMAX=Results$maxL,L=Results$L,ENTR=Results$ENTR,LAM=Results$LAM,TT=Results$TT)
...
fitCrqa2 <- function(ts1,ts2){
  ...
  if (length(Pars)>1){
    Results=crqa(ts1, ts2, delay=18, embed=2, radius=0.38, normalize=0, rescale=0, mindiagline = 2, minvertline = 2)
   } 
    # else {
  #   Results=NULL
  #   Pars=NULL
...
for (i in files){
  print(i)
  d2 = read.csv(i)
  resp_df2 = fitCrqa2(d2$Resp1S,d2$Resp2S)
  heart_df2 = fitCrqa1(d2$HR1S,d2$HR2S)
  ...
  df2 = data.frame(Study, Group, Trial, Condition,
                  RespRR=resp_df$RR, RespDET = resp_df$DET, RespMaxL = resp_df$LMAX,RespL=resp_df$L, 
                  RespEntr = resp_df$ENTR, RespLAM = resp_df$LAM, RespTT=resp_df$TT, 
                  HeartRR = heart_df$RR, HeartDET = heart_df$DET, HeartMaxL = heart_df$LMAX, 
                  HeartL = heart_df$L, HeartEntr = heart_df$ENTR, HeartLAM = heart_df$LAM, 
  ...
  if(exists("dataframe2")){dataframe2 = rbind(dataframe2, df2)} else {dataframe2 = df2}
